...
9. Before pushing: run `/rocketmind:review` on all changed files. Address every CRITICAL and HIGH finding before proceeding. Record the ship decision.
...
- **Never push without running `/rocketmind:review` first** — self-review is mandatory, not optional
