...
function buildTaskBoundaryWorkflow(args, evidence) {
  ...
  return {
    ...
    nextCommand: `/rocketmind:quick ${requestedIssue}`,
    blockers: [
...
function buildRuntimeCommandOutput(args, profile) {
  ...
  const why =
    ...
    profile.defaultWhy ||
    `RocketMind classified this as ${profile.command} and emitted the next workflow step.`;
  const prLabel =
    args.pr ||
    args.prNumber ||
    evidence.prNumber ||
  ...
  sections.push(
    formatNextCommand({
      primary,
      why,
      alternatives: profile.alternatives || ['/rocketmind:progress', '/rocketmind:resume'],
    })
  );
