...
const DEFAULT_STATE_PATH = path.join(ROOT, '.rocketmind', 'state', 'STATE.md');
...
function resolveNextAction(args = {}, deps = {}) {
  ...
  if (isTrackedBranch && HANDOFF_COMPLETE_STATES.has(workflow.state) && nextIssue) {
    ...
    return {
      ...
      workflow: {
        ...
        nextCommand: `/rocketmind:quick ${nextIssue.issue} ${title}`,
        blockers: [
      ...
      primary: `/rocketmind:quick ${nextIssue.issue} ${title}`,
  ...
  if (isTrackedBranch) {
    return {
      workflow,
      primary: workflow.nextCommand || '/rocketmind:resume',
      why:
        workflow.blockers[0] ||
        `RocketMind resolved the next step from active branch ${branch} and tracked issue state.`,
