...
/**
 * RocketMind Safety Evaluator v1.0
...
const _registry = JSON.parse(
  fs.readFileSync(path.join(projectRoot, 'rocketmind.registry.json'), 'utf8')
);
...
function findProjectRoot(start) {
  ...
  while (curr !== path.parse(curr).root) {
    if (fs.existsSync(path.join(curr, 'rocketmind.registry.json'))) return curr;
