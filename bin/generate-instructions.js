#!/usr/bin/env node
/**
 * RocketMind Instruction Generator
 *
 * Two modes:
 *
 * 1. Runtime instruction file (default):
 *    Generates a runtime-specific instruction file from templates/rocketmind.base.md.
 ...
 *    rocketmind.registry.json + their corresponding templates/*.tpl.md.
 *    Run this after any change to the registry or the .tpl.md templates.
 *    CI (rocketmind-sentinel.yml) checks these files for drift.
 *
 *    Usage:
...
if (!runtime && !humanViews) {
  ...
  console.error('');
  console.error('Runtimes are defined in rocketmind.config.json → runtimes');
  process.exit(1);
}
...
const configPath = path.join(frameworkRoot, 'rocketmind.config.json');
const registryPath = path.join(frameworkRoot, 'rocketmind.registry.json');
const templatePath = path.join(frameworkRoot, 'templates', 'rocketmind.base.md');

if (!fs.existsSync(configPath)) {
  console.error('❌ rocketmind.config.json not found');
  process.exit(1);
}
...
if (humanViews) {
  if (!fs.existsSync(registryPath)) {
    console.error('❌ rocketmind.registry.json not found');
    process.exit(1);
  }
...
if (!fs.existsSync(templatePath)) {
  console.error('❌ templates/rocketmind.base.md not found');
  process.exit(1);
}
...
const implicitRoutingRule = implicitPromptRouting
  ? `This runtime supports RocketMind workflow inference for plain prompts via ${routingMechanism}. If the user does not explicitly prefix a request with an RocketMind command, infer the nearest workflow and continue through that boundary. Explicit \`/rocketmind:*\` commands still take precedence.`
  : `This runtime does not provide reliable plain-prompt interception. For tracked work, prefer explicit \`/rocketmind:*\` commands or the runtime's documented RocketMind workflow equivalent. If a plain prompt arrives anyway, route conservatively and do not overstate native workflow interception.`;
const implicitRoutingBullet = implicitPromptRouting
  ? 'If the user gives a plain prompt instead of a slash command, infer the correct RocketMind workflow and act through that workflow boundary.'
  : "If the user gives a plain prompt instead of a slash command, prefer the runtime's documented explicit RocketMind command path because this runtime does not guarantee plain-prompt interception.";
