...
const DEFAULT_STATE_DIR = path.join(ROOT, '.rocketmind', 'state');
const DEFAULT_ERROR_DIR = path.join(ROOT, '.rocketmind', 'errors');
...
function recordRecoveryAttempt(options = {}) {
  ...
  const command = options.command || '/rocketmind:riper';
  ...
  const nextCommand = decision === 'halt' ? '/rocketmind:review' : command;
