...
const DEFAULT_CHECKPOINT_DIR = path.join(ROOT, '.rocketmind', 'state', 'checkpoints');
...
function categorizeImpact(changedFiles) {
  ...
  return {
    logic: files.some(
      (file) =>
        /^(bin|agents|hooks\/scripts|skills|commands\/rocketmind)\//.test(file) &&
        !/\.test\./.test(file) &&
        !/\.md$/.test(file)
    ),
    ...
    config: files.some((file) =>
      /(^|\/)(package\.json|rocketmind\.registry\.json|rocketmind\.config\.json|rocketmind\.config\.schema\.json)$/.test(
        file
      )
    ),
  };
...
function buildCheckpointManifest({ args = {}, profile = {}, evidence = {}, workflow = {} }) {
  ...
  return {
    rocketmind_version: args.rocketmindVersion || 'current',
    checkpoint:
    ...
    impact: {
      ...
      registry_updated: files.includes('rocketmind.registry.json'),
      breaking_changes:
