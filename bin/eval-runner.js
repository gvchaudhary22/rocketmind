#!/usr/bin/env node
/**
 * RocketMind Eval Runner (Issue #13)
...
function buildFakeRuntime({ branch, dirty = false, prData = null }) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-eval-runtime-'));
...
function installRuntimeArtifact(tool) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), `rocketmind-eval-install-${tool}-`));
  try {
    execFileSync(
      'bash',
...
function initGitRepo(repoDir) {
  ...
  execFileSync('git', ['config', 'user.name', 'RocketMind Test'], {
    cwd: repoDir,
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  execFileSync('git', ['config', 'user.email', 'rocketmind-tests@example.com'], {
    cwd: repoDir,
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'pipe'],
  });
...
function runInstallOrSetup({ kind, worktree = false }) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), `rocketmind-eval-${kind}-`));
  try {
...
function parseEvalDataset(content) {
  ...
  for (const line of lines) {
    ...
    if (inTable && headerPassed && cells.length >= 5) {
      const id = cells[0];
      const agent = cells[3];
      // Extract first /rocketmind:command from the workflow cell (strip backticks/qualifiers)
      const wfCell = cells[4];
      const wfMatch = wfCell.match(/`(\/rocketmind:[a-z-]+)`/);
...
const registryText = readFile('rocketmind.registry.json');
if (!registryText) {
  console.error('ERROR: rocketmind.registry.json not found');
  process.exit(2);
}
...
const REQUIRED_V29_WORKFLOWS = [
  { command: '/rocketmind:ask', doc: 'commands/rocketmind/ask.md' },
  { command: '/rocketmind:clarify', doc: 'commands/rocketmind/clarify.md' },
  { command: '/rocketmind:discover', doc: 'commands/rocketmind/discover.md' },
  { command: '/rocketmind:eval', doc: 'commands/rocketmind/eval.md' },
  { command: '/rocketmind:riper', doc: 'commands/rocketmind/riper.md' },
];
...
const workflowResults = evalCases.map((c) => {
  if (!c.workflow) {
    return {
      ...
      workflow: null,
      pass: false,
      reason: 'no /rocketmind: command parsed from dataset',
    };
  }
...
// All agent files exist, all skill files exist, all workflow commands are /rocketmind:.
...
for (const wf of registry.workflows) {
  const valid = /^\/rocketmind:[a-z-]+$/.test(wf.command);
  integrityResults.push({
...
const shipWorkflow = registry.workflows.find((entry) => entry.command === '/rocketmind:ship');
...
integrityResults.push(
  ...
  {
    check: 'workflow contract: /rocketmind:ship references agents',
    ...
    reason:
      Array.isArray(shipWorkflow?.agents) && shipWorkflow.agents.length > 0
        ? 'ok'
        : '/rocketmind:ship must declare agent refs',
  },
  {
    check: 'workflow contract: /rocketmind:ship includes technical-writer',
    ...
    reason:
      Array.isArray(shipWorkflow?.agents) && shipWorkflow.agents.includes('technical-writer')
        ? 'ok'
        : '/rocketmind:ship must include technical-writer',
  },
  {
...
integrityResults.push({
  ...
  reason:
    typeof config.loop_detection?.enabled === 'boolean'
      ? 'ok'
      : 'rocketmind.config.json missing loop_detection.enabled boolean',
});
integrityResults.push({
  ...
  reason:
    Number.isInteger(config.loop_detection?.window_size) && config.loop_detection.window_size > 0
      ? 'ok'
      : 'rocketmind.config.json missing positive loop_detection.window_size',
});
integrityResults.push({
  ...
  reason:
    Number.isInteger(config.loop_detection?.threshold) && config.loop_detection.threshold > 0
      ? 'ok'
      : 'rocketmind.config.json missing positive loop_detection.threshold',
});
integrityResults.push({
  ...
  reason:
    typeof config.clarification_gate === 'boolean'
      ? 'ok'
      : 'rocketmind.config.json missing clarification_gate boolean',
});
integrityResults.push({
  ...
  reason:
    typeof config.distributed_mutex_warning === 'boolean'
      ? 'ok'
      : 'rocketmind.config.json missing distributed_mutex_warning boolean',
});
integrityResults.push({
  ...
  pass:
    ...
    preToolUseHookText.includes('/rocketmind:clarify'),
  reason:
    preToolUseHookText.includes('bin/clarification-gate.js') &&
    preToolUseHookText.includes('/rocketmind:clarify')
      ? 'ok'
...
integrityResults.push({
  ...
  pass:
    ...
    repoStateText.includes('**Directive**: This release is executed by RocketMind'),
  reason:
    repoStateText.includes('**Active Milestone**: v2.9.0 — Idea to Market') &&
    repoStateText.includes('**Directive**: This release is executed by RocketMind')
      ? 'ok'
      : '.rocketmind/state/STATE.md missing v2.9.0 release milestone or directive metadata',
});
integrityResults.push({
  ...
  ...(() => {
    ...
    return {
      ...
      reason: pass
        ? 'ok'
        : `.rocketmind/state/STATE.md expected 20 tracked issues and 6 wave headings, found ${issueRefs.length} issues / ${waveCount} wave headings`,
    };
  })(),
});
integrityResults.push({
...
const promptRoutingCapabilityResults = [
  ...
  },
  {
    ...
    pass: adapterText.includes('Plain-prompt RocketMind workflow routing is not currently supported'),
    reason: adapterText.includes('Plain-prompt RocketMind workflow routing is not currently supported')
      ? 'ok'
  ...
  {
    check: 'prompt dispatch helper exists for strict top-level RocketMind command enforcement',
    pass: fileExists('bin/prompt-dispatch.js'),
    reason: fileExists('bin/prompt-dispatch.js') ? 'ok' : 'missing bin/prompt-dispatch.js',
  },
  {
    check: 'prompt dispatch helper makes explicit RocketMind commands override inference',
    ...(() => {
      try {
        const result = JSON.parse(
          runNode('bin/prompt-dispatch.js', ['--prompt', 'rocketmind:quick #145'])
        );
        const pass =
          result.type === 'explicit_command' &&
          result.command === '/rocketmind:quick' &&
          result.remainder === '#145';
        return {
          pass,
          reason: pass ? 'ok' : 'explicit RocketMind command did not dispatch to /rocketmind:quick',
        };
      } catch (error) {
        return {
          pass: false,
  ...
  {
    check: 'prompt dispatch helper can execute repo-local explicit rocketmind review commands',
    ...(() => {
      try {
        const output = runNode('bin/prompt-dispatch.js', [
          '--prompt',
          'rocketmind:review on PR #189',
          '--execute',
          '--pr',
          '#189',
        ]);
        const pass = output.includes('Command:  /rocketmind:review');
        return {
          pass,
          reason: pass
            ? 'ok'
            : 'prompt-dispatch execute mode did not route to /rocketmind:review output',
        };
      } catch (error) {
        return {
          pass: false,
...
const runtimeEnforcementResults = [
    ...
    ...(() => {
      ...
      try {
        ...
        const pass =
          ...
          output.includes('Command:  /rocketmind:review');
        return {
          pass,
          reason: pass
            ? 'ok'
  ...
  },
  {
    ...
    ...(() => {
      ...
      try {
        ...
        const pass =
          stderr.includes('Pull request gate blocked') && stderr.includes('/rocketmind:review');
        return {
          pass,
  ...
  },
  {
    ...
    ...(() => {
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-eval-recovery-'));
      const stateDir = path.join(tmpDir, '.rocketmind', 'state');
      try {
  ...
  },
  {
    ...
    ...(() => {
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-eval-riper-auto-'));
      const stateDir = path.join(tmpDir, '.rocketmind', 'state');
      try {
  ...
  },
  {
    ...
    ...(() => {
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-eval-routing-'));
      ...
      try {
        ...
        const pass =
          codexText.includes('supports RocketMind workflow inference for plain prompts') &&
...
const OBS_CHECKS = [
  {
    id: 'OBS001',
    check: 'rocketmind:quick emits classification header (━━━ RocketMind)',
    section: '/rocketmind:quick',
  },
  {
    id: 'OBS002',
    check: 'rocketmind:build emits classification header (━━━ RocketMind)',
    section: '/rocketmind:build',
  },
  {
    id: 'OBS003',
    check: 'rocketmind:plan emits classification header (━━━ RocketMind)',
    section: '/rocketmind:plan',
  },
  {
    id: 'OBS004',
    check: 'rocketmind:build emits wave completion block (━━━ Wave)',
    section: '/rocketmind:build',
    keyword: 'Wave {N} Complete',
  },
  {
    id: 'OBS005',
    check: 'rocketmind:progress emits current execution block',
    section: '/rocketmind:progress',
    keyword: '━━━ Current Execution',
  },
  {
    id: 'OBS006',
    check: 'rocketmind:resume emits recommended next command footer',
    section: '/rocketmind:resume',
    keyword: '## Recommended Next Command',
  },
  {
    id: 'OBS007',
    check: 'rocketmind:quick runtime emits the standard status blocks',
    ...checkRuntimeCommandOutput(
      'bin/quick.js',
      ['--issue', '#146', '--branch', 'feat/146-runtime-status-parity'],
      ['━━━ RocketMind', 'Current Execution', 'Workflow Gate', '## Recommended Next Command']
    ),
  },
  {
    id: 'OBS008',
    check: 'rocketmind:plan runtime emits the standard status blocks',
    ...checkRuntimeCommandOutput(
      'bin/plan.js',
      ['--issue', '#146', '--branch', 'feat/146-runtime-status-parity'],
      ['━━━ RocketMind', 'Current Execution', 'Workflow Gate', '## Recommended Next Command']
  ...
  {
    id: 'OBS009',
    check: 'rocketmind:review runtime emits the standard status blocks',
    ...checkRuntimeCommandOutput(
      'bin/review.js',
      ['--issue', '#146', '--branch', 'feat/146-runtime-status-parity'],
      ['━━━ RocketMind', 'Current Execution', 'Workflow Gate', '## Recommended Next Command']
  ...
  {
    id: 'OBS010',
    check: 'rocketmind:verify runtime emits the standard status blocks',
    ...checkRuntimeCommandOutput(
      'bin/verify.js',
      ['--issue', '#146', '--branch', 'feat/146-runtime-status-parity'],
      ['━━━ RocketMind', 'Current Execution', 'Workflow Gate', '## Recommended Next Command']
  ...
  {
    id: 'OBS011',
    check: 'rocketmind:next runtime emits the standard status blocks',
    ...checkRuntimeCommandOutput(
      'bin/next.js',
      ['--issue', '#146', '--branch', 'feat/146-runtime-status-parity'],
      ['━━━ RocketMind', 'Current Execution', 'Workflow Gate', '## Recommended Next Command']
  ...
  {
    id: 'OBS012',
    check: 'rocketmind:riper runtime emits the standard status blocks',
    ...checkRuntimeCommandOutput(
      'bin/riper.js',
  ...
  {
    id: 'OBS013',
    check: 'rocketmind:clarify runtime emits the standard status blocks',
    ...checkRuntimeCommandOutput(
      'bin/clarify.js',
  ...
  {
    id: 'OBS015',
    check: 'rocketmind:quick runtime can emit a checkpoint manifest block',
    ...(() => {
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-eval-checkpoint-'));
      try {
        const output = runNode('bin/quick.js', [
          ...
          'abc1234',
          '--changedFiles',
          'bin/runtime-command.js,tests/command-runtime.test.js,docs/quality/evaluation-framework.md,rocketmind.registry.json',
          '--verificationChecks',
        ...
        const pass =
          output.includes('RocketMind Auto-Chain') &&
          output.includes('RocketMind Checkpoint') &&
...
const observabilityResults = OBS_CHECKS.map((obs) => {
  ...
  const sectionStart = commandsText.indexOf(`# RocketMind Command: ${obs.section}`);
  const nextSection = commandsText.indexOf('\n# RocketMind Command:', sectionStart + 1);
  const sectionText =
    sectionStart >= 0
      ? commandsText.slice(sectionStart, nextSection > 0 ? nextSection : undefined)
      : '';
  const keyword = obs.keyword || '━━━ RocketMind';
...
if (JSON_OUT) {
  ...
  console.log(`\n${B}RocketMind Eval Runner v${registry.version}${N}`);
