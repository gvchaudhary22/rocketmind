...
const EXECUTABLE_COMMANDS = new Map([
  ['/rocketmind:quick', (args, deps) => (deps.runQuick || runQuick)(args, deps).output],
  ['/rocketmind:plan', (args, deps) => (deps.renderPlan || renderPlan)(args)],
  ['/rocketmind:review', (args, deps) => (deps.renderReview || renderReview)(args)],
  ['/rocketmind:verify', (args, deps) => (deps.renderVerify || renderVerify)(args)],
  ['/rocketmind:next', (args, deps) => (deps.renderNext || renderNext)(args)],
  ['/rocketmind:riper', (args, deps) => (deps.renderRiper || renderRiper)(args)],
  ['/rocketmind:clarify', (args, deps) => (deps.renderClarify || renderClarify)(args)],
]);
...
function extractExplicitRocketMindCommand(prompt) {
  const normalized = normalizePrompt(prompt);
  const match = normalized.match(/^(?:\/)?(rocketmind:[a-z-]+)\b(.*)$/i);
  ...
  return {
    ...
    raw: normalized,
    remainder,
    reason: 'Explicit RocketMind command detected; inferred routing and freeform handling are disabled.',
  };
}
...
function inferWorkflow(prompt, context = {}) {
  ...
  if (
    ...
    return {
      type: 'inferred_workflow',
      command: `/rocketmind:quick ${activeIssue}`,
  ...
  if (/^(review this|please review)\b/.test(normalized)) {
    return {
      ...
      reason: 'Review prompt detected; route through RocketMind review.',
    };
  }
  ...
  return {
    type: 'unclassified',
    command: null,
    reason: 'No explicit RocketMind command or supported plain-prompt workflow match found.',
  };
}

function classifyPromptDispatch(prompt, context = {}) {
  const explicit = extractExplicitRocketMindCommand(prompt);
...
function dispatchPrompt(prompt, context = {}, deps = {}) {
  ...
  if (!execute) {
    return {
      ...
      executed: false,
      output: null,
      error: `Explicit RocketMind command ${dispatch.command} is not executable through the repo-local dispatcher yet.`,
    };
  }
...
module.exports = {
  ...
  dispatchPrompt,
  EXECUTABLE_COMMANDS,
  extractExplicitRocketMindCommand,
  inferWorkflow,
  normalizePrompt,
  parseArgs,
};
