#!/usr/bin/env node
/**
 * bin/context.js — RocketMind context.db CLI
...
const STATE_PATH = path.join(ROOT, '.rocketmind', 'state', 'STATE.md');
const DEFAULT_CHECKPOINT_PATH = path.join(ROOT, '.rocketmind', 'state', 'checkpoints', 'latest.json');
...
function latestCheckpointPath() {
  return process.env.ROCKETMIND_CHECKPOINT_PATH || DEFAULT_CHECKPOINT_PATH;
}
...
function main() {
  ...
  if (flag === '--load' && !fs.existsSync(DB_PATH)) {
    console.error('ERROR: context.db not found at .rocketmind/context.db');
