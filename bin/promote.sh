#!/usr/bin/env bash
# RocketMind Promote — /rocketmind:promote
# Validates a local forged agent or skill and prints a PR draft for upstreaming to shiprocket/rocketmind.
...
#   3. Checks it does not already exist in the RocketMind core
#   4. Prints a PR body ready to open against shiprocket/rocketmind
...
echo -e "${BOLD}RocketMind Promote — validating: $CANDIDATE${NC}"
echo ""
...
if [[ -f "$CORE_FILE" ]]; then
  echo -e "${YELLOW}⚠  $DEST_DIR/$NAME.md already exists in the RocketMind core.${NC}"
...
Body:
## Summary
- Promotes \`${TYPE}/${NAME}.md\` from \`${SOURCE_REPO}\` to the RocketMind core
- Type: ${TYPE}
...
- [ ] rocketmind.registry.json updated to include ${NAME}
...
echo ""
echo "  1. Fork or branch shiprocket/rocketmind"
echo "  2. Copy $CANDIDATE → $DEST_DIR/$NAME.md"
echo "  3. Add to rocketmind.registry.json"
...
if [[ "$DRY_RUN" -eq 0 ]] && command -v gh &>/dev/null; then
  ...
  echo "  gh pr create --repo shiprocket/rocketmind --title \"promote(${TYPE}): add ${NAME}\" --body \"...\""
fi
