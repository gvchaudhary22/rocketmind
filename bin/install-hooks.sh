#!/usr/bin/env bash
# RocketMind Hook Installation — Links repo hooks to the active Git hooks directory
...
if ! command -v git >/dev/null 2>&1; then
  echo "⚠️  Git is not available. Skipping RocketMind hook installation."
  exit 0
fi

if ! git -C "$PROJECT_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  echo "⚠️  No Git worktree found at $PROJECT_DIR. Skipping RocketMind hook installation."
  exit 0
fi
...
echo "🛡️  Installing RocketMind Governance Hooks..."
...
echo "✅ RocketMind hooks installed in $HOOK_TARGET_DIR. Governance is now enforced on commits and pushes."
