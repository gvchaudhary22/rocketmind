...
function stateFileFromArgs(args = {}) {
  return args['state-file'] || args.stateFile || path.join(ROOT, '.rocketmind', 'state', 'STATE.md');
}
...
function renderClarify(args = {}) {
  ...
  const base = buildRuntimeCommandOutput(args, {
    command: '/rocketmind:clarify',
    domain: 'OPERATIONS',
    complexity: 'TASK',
    agent: 'strategist',
    progressAgent: 'strategist',
    defaultPrimary: pending.length > 0 ? '/rocketmind:clarify' : '/rocketmind:next',
    defaultWhy:
      pending.length > 0
        ? 'There are unresolved clarification requests blocking safe autonomous execution.'
        : 'No clarification requests are pending, so RocketMind can continue normal workflow progression.',
    ...
    workflowOverride:
      pending.length > 0
        ? {
            ...
            nextCommand: '/rocketmind:clarify',
            blockers: [
