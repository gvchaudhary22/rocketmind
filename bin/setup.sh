#!/usr/bin/env bash
# bin/setup.sh — RocketMind unified installer
# Auto-detects installed coding agents and sets up RocketMind for each.
# Usage:  npm run setup
#         npm run setup -- --tool claude
#         npm run setup -- --tool claude,codex
set -euo pipefail

FRAMEWORK_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ROCKETMIND_DIR="$FRAMEWORK_DIR/.rocketmind"
ADAPTERS_DIR="$ROCKETMIND_DIR/adapters"

# ── Colour helpers ─────────────────────────────────────────────────────────────
G='\033[0;32m'
Y='\033[1;33m'
R='\033[0;31m'
B='\033[1m'
N='\033[0m'

# ── Tool detection ─────────────────────────────────────────────────────────────

detect_tools() {
  local tools=()
  command -v claude      &>/dev/null && tools+=("claude")
  command -v codex       &>/dev/null && tools+=("codex")
  [[ -d "$HOME/.antigravity" ]] && tools+=("antigravity")
  echo "${tools[*]:-}"
}

# ── Adapter directory setup ────────────────────────────────────────────────────

create_adapter_dir() {
  local tool="$1"
  local adapter_path="$ADAPTERS_DIR/$tool"
  mkdir -p "$adapter_path"

  local config_ref
  case "$tool" in
    claude)      config_ref=".claude/CLAUDE.md (generated by install.sh)" ;;
    codex)       config_ref=".codex/INSTRUCTIONS.md (generated by install.sh)" ;;
    antigravity) config_ref=".antigravity/CLAUDE.md (generated by install.sh)" ;;
    *)           config_ref="(unknown config path)" ;;
  esac

  local hook_support
  case "$tool" in
    claude) hook_support="✅" ;;
    *)      hook_support="❌" ;;
  esac

  local prompt_routing
  case "$tool" in
    claude|codex) prompt_routing="supported" ;;
    *)            prompt_routing="explicit commands required" ;;
  esac

  # Use single-quoted EOF to prevent shell expansion in the heredoc
  cat > "$adapter_path/README.md" << 'HEREDOC_END'
# RocketMind Adapter

This directory marks the RocketMind adapter for this tool.

Generated config files are created by `install.sh`. Do not hand-edit.
Re-run `npm run setup` to regenerate.
HEREDOC_END

  # Append dynamic content separately
  {
    echo ""
    echo "## Tool: ${tool}"
    echo "Config: \`${config_ref}\`"
    echo ""
    echo "## Hook support"
    echo ""
    echo "| Hook        | Support |"
    echo "|-------------|---------|"
    echo "| PreToolUse  | ${hook_support} |"
    echo "| PostToolUse | ${hook_support} |"
    echo "| PreCompact  | ${hook_support} |"
    echo "| Stop        | ${hook_support} |"
    echo ""
    echo "## Plain-prompt routing"
    echo ""
    echo "- ${prompt_routing}"
  } >> "$adapter_path/README.md"

  node "$FRAMEWORK_DIR/bin/runtime-adapter.js" \
    --runtime "$tool" --output "$adapter_path/adapter.contract.json" >/dev/null

  echo "$adapter_path"
}

# ── Install one tool ────────────────────────────────────────────────────────────

install_tool() {
  local tool="$1"
  case "$tool" in
    claude)
      bash "$FRAMEWORK_DIR/install.sh" --local --tool claude --quiet
      ;;
    codex)
      bash "$FRAMEWORK_DIR/install.sh" --local --tool codex --quiet
      ;;
    antigravity)
      bash "$FRAMEWORK_DIR/install.sh" --local --tool antigravity --quiet
      ;;
    *)
      echo "  Unknown tool: $tool — skipping"
      return 1
      ;;
  esac
}

ensure_repo_hooks() {
  if bash "$FRAMEWORK_DIR/bin/install-hooks.sh" --project-dir "$FRAMEWORK_DIR" >/dev/null 2>&1; then
    echo -e "  ${G}✓${N} git lifecycle hooks active"
  else
    echo -e "  ${Y}⚠${N} git lifecycle hooks skipped"
    echo "     Run manually: bash bin/install-hooks.sh --project-dir \"$FRAMEWORK_DIR\""
  fi
}

# ── Main ───────────────────────────────────────────────────────────────────────

main() {
  local explicit_tools=""

  # Parse flags
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --tool)
        explicit_tools="$2"
        shift 2
        ;;
      --tool=*)
        explicit_tools="${1#--tool=}"
        shift
        ;;
      -h|--help)
        echo "Usage: npm run setup [-- --tool <tool1,tool2>]"
        echo "  --tool   Comma-separated list of tools (claude, codex, antigravity)"
        echo "           If omitted, auto-detects installed tools"
        exit 0
        ;;
      *)
        echo "Unknown flag: $1" >&2
        echo "Usage: npm run setup [-- --tool claude,codex]" >&2
        exit 1
        ;;
    esac
  done

  echo ""
  echo -e "${B}━━━ RocketMind Setup ━━━━━━━━━━━━━━━━━━━━━━━━${N}"
  echo ""

  # Determine tools to install
  local tools_str
  if [[ -n "$explicit_tools" ]]; then
    tools_str="${explicit_tools//,/ }"
  else
    tools_str="$(detect_tools)"
  fi

  # Convert to array safely
  local tools=()
  if [[ -n "$tools_str" ]]; then
    IFS=' ' read -ra tools <<< "$tools_str"
  fi

  if [[ ${#tools[@]} -eq 0 ]]; then
    echo -e "  ${Y}⚠️  No coding agents detected.${N}"
    echo "     Install Claude Code, Codex, or Antigravity first."
    echo "     Then re-run: npm run setup"
    echo ""
    exit 0
  fi

  # Track results using temp files
  local tmp_dir
  tmp_dir="$(mktemp -d)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp_dir'" EXIT

  for tool in "${tools[@]}"; do
    # Run each tool install in a subshell so one failure doesn't abort others
    if (install_tool "$tool") 2>&1; then
      local adapter_abs
      adapter_abs="$(create_adapter_dir "$tool")"
      local adapter_rel="${adapter_abs#$FRAMEWORK_DIR/}"
      echo "$adapter_rel" > "$tmp_dir/${tool}.ok"
      echo -e "  ${G}✓${N} ${tool}"
    else
      echo "failed" > "$tmp_dir/${tool}.fail"
      echo -e "  ${R}✗${N} ${tool} — install failed, continuing"
    fi
  done
  echo ""

  # Print summary table
  echo -e "  ${B}Tool         Detected   Installed   Adapter${N}"
  echo "  ─────────────────────────────────────────────────────────"

  for tool in claude codex antigravity; do
    local detected="✗"
    local installed="—"
    local adapter_col="—"

    for t in "${tools[@]}"; do
      if [[ "$t" == "$tool" ]]; then
        detected="✓"
        if [[ -f "$tmp_dir/${tool}.ok" ]]; then
          installed="✓"
          adapter_col="$(cat "$tmp_dir/${tool}.ok")"
        fi
        break
      fi
    done

    printf "  %-12s %-10s %-11s %s\n" "$tool" "$detected" "$installed" "$adapter_col"
  done

  echo ""

  # ── Bootstrap context.db ────────────────────────────────────────────────────
  # Always run after install so context.db reflects the current repo state.
  # --force overwrites any existing db so setup is always idempotent.
  echo -e "${B}Bootstrapping context.db...${N}"
  if node "$FRAMEWORK_DIR/bin/bootstrap.js" --force 2>&1; then
    echo -e "  ${G}✓ context.db ready at .rocketmind/context.db${N}"
  else
    echo -e "  ${Y}⚠  bootstrap failed — context.db may be missing or stale${N}"
    echo "     Run manually: npm run bootstrap"
  fi

  echo ""
  echo -e "${B}Ensuring git lifecycle hooks...${N}"
  ensure_repo_hooks

  echo ""
  echo -e "${B}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${N}"
  echo -e "  ${G}Setup complete.${N}"
  echo ""
  echo -e "  ${B}What to do next:${N}"
  echo ""

  # Tool-specific next steps
  for tool in "${tools[@]}"; do
    case "$tool" in
      claude)
        echo -e "  ${G}Claude Code (VS Code):${N}"
        echo "    Open this folder in VS Code — RocketMind context loads automatically."
        echo "    First command: /rocketmind:resume"
        echo ""
        ;;
      codex)
        echo -e "  ${G}Codex:${N}"
        echo "    Run:  codex"
        echo "    Then: Read .codex/policy.md and .codex/INSTRUCTIONS.md to load RocketMind context."
        echo "          Read .rocketmind/state/STATE.md for project state."
        echo "          Then: /rocketmind:resume"
        echo ""
        ;;
      antigravity)
        echo -e "  ${G}Antigravity:${N}"
        echo "    Open this folder in Antigravity."
        echo "    RocketMind reads: .antigravity/CLAUDE.md (auto-loaded at session start)"
        echo "    First command: /rocketmind:resume"
        echo ""
        ;;
    esac
  done

  echo -e "  ${B}Verify context:${N}"
  echo "    node bin/context.js --load minimal"
  echo ""
  echo -e "${B}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${N}"
  echo ""
}

main "$@"
