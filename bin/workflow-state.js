#!/usr/bin/env node
/**
 * RocketMind workflow state engine.
...
const NEXT_COMMANDS = {
  untracked: () => '/rocketmind:plan 1',
  issue_ready: (evidence) => `/rocketmind:quick ${evidence.issue || '#NNN'}`,
  branch_ready: (evidence) => `/rocketmind:quick ${evidence.issue || '#NNN'}`,
  implementation_in_progress: (evidence) => `/rocketmind:quick ${evidence.issue || '#NNN'}`,
  implementation_done: (evidence) => `/rocketmind:quick ${evidence.issue || '#NNN'}`,
  tests_green: () => '/rocketmind:review',
  review_required: () => '/rocketmind:review',
  remediation_required: (evidence) => `/rocketmind:quick ${evidence.issue || '#NNN'}`,
  review_clean: () => '/rocketmind:ship',
  pr_ready: () => '/rocketmind:ship',
  pr_open: () => '/rocketmind:progress',
  merged: () => '/rocketmind:resume',
};
...
function evaluateWorkflowState(rawEvidence = {}) {
  ...
  if (evidence.reviewStatus === 'approved' && evidence.reviewEvidenceStatus === 'missing') {
    blockers.push(
      'Review evidence missing; refresh the PR body with RocketMind self-review details before shipping.'
    );
  ...
  if (evidence.shipDecisionStatus === 'blocked') {
    blockers.push(
      'RocketMind self-review is blocked; resolve the recorded findings before PR or ship progression.'
    );
    return buildSummary('review_required', evidence, blockers);
  }
...
function assertWorkflowTransition(targetState, evidence = {}) {
  ...
  if (!WORKFLOW_STATES.includes(targetState)) {
    throw new Error(
      `[ERR-ROCKETMIND-007] Unknown workflow transition target "${targetState}". Allowed states: ${WORKFLOW_STATES.join(', ')}`
    );
  }
  ...
  if (!summary.allowedTransitions.includes(targetState)) {
    ...
    throw new Error(
      `[ERR-ROCKETMIND-007] Cannot move from ${summary.state} to ${targetState}. Next allowed transition: ${summary.nextTransition || 'none'}.${blockers}`
    );
  }
...
function assertPullRequestReady(evidence = {}) {
  ...
  if (!summary.canOpenPullRequest) {
    ...
    throw new Error(
      `[ERR-ROCKETMIND-007] Pull request gate blocked at ${summary.state}. Next: ${summary.nextTransition || 'none'}.${blockers}`
    );
  }
  return summary;
}
