...
function renderProgress(args = {}) {
  ...
  const command = args.command || '/rocketmind:progress';
  ...
  return [
    ...
    formatNextCommand({
      primary: workflow.nextCommand || '/rocketmind:progress',
      why: workflow.blockers[0] || `Workflow state is ${workflow.state}.`,
      alternatives: ['/rocketmind:resume', '/rocketmind:review'],
    }),
  ].join('\n\n');
}
