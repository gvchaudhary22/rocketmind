...
required_files=(
  ...
  "rocketmind.config.schema.json"
  "docs/architecture/runtime-adapters.md"
  "docs/quality/evaluation-framework.md"
  "docs/quality/eval-dataset.md"
  "commands/commands.md"
  "commands/rocketmind/eval.md"
  "commands/rocketmind/clarify.md"
  "commands/rocketmind/riper.md"
  "hooks/HOOKS.md"
  "install.sh"
  "package.json"
  "rocketmind.config.json"
  "templates/STATE.md"
  "templates/OPERATIONAL-RULES.json"
  ".rocketmind/state/STATE.md"
...
const configPath = path.join(root, 'rocketmind.config.json');
const registryPath = path.join(root, 'rocketmind.registry.json');
const schemaPath = path.join(root, 'rocketmind.config.schema.json');
...
const evalCommandPath = path.join(root, 'commands', 'rocketmind', 'eval.md');
...
if (!fs.existsSync(evalCommandPath)) {
  throw new Error('missing /rocketmind:eval command wrapper');
}
...
for (const workflow of registry.workflows || []) {
  if (typeof workflow.command !== 'string' || !workflow.command.startsWith('/rocketmind:')) {
...
printf 'RocketMind validation passed.\n'
...
# Check templates/rocketmind.base.md — the canonical source. CLAUDE.md is generated at install time.
MODEL_PATTERN='claude-(haiku|sonnet|opus)-[0-9]'
if grep -qE "$MODEL_PATTERN" "$ROOT_DIR/templates/rocketmind.base.md"; then
  printf '\n⚠️  Warning: templates/rocketmind.base.md contains hardcoded model IDs.\n' >&2
  printf '   Model versions should live only in rocketmind.config.json → models.routing.\n' >&2
  grep -nE "$MODEL_PATTERN" "$ROOT_DIR/templates/rocketmind.base.md" >&2 || true
fi
