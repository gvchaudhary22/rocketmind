...
class RocketMindOrchestrator {
  constructor(projectRoot) {
    this.projectRoot = projectRoot;
    this.registryPath = path.join(projectRoot, 'rocketmind.registry.json');
    this.configPath = path.join(projectRoot, 'rocketmind.config.json');
    this.stateDir = path.join(projectRoot, '.rocketmind', 'state');
    ...
    this.nexusPath = path.join(projectRoot, 'rocketmind.nexus.json');
  ...
  emitDistributedMutexWarning() {
    ...
    console.warn(
      '[WARN-ROCKETMIND-DISTRIBUTED-MUTEX] RocketMind state locking uses a local .rocketmind.lock directory and is not safe across distributed CI runners or remote subagent hosts. STATE.md writes remain local-only in this mode; do not assume cross-runner mutual exclusion.'
    );
  ...
  acquireStateLock() {
    ...
    const lockPath = path.join(this.stateDir, '.rocketmind.lock');
    let attempts = 0;
    while (attempts < 10) {
      try {
    ...
    throw new Error(
      '[ERR-ROCKETMIND-003] Could not acquire RocketMind state lock after 10 retries. Delete .rocketmind/state/.rocketmind.lock if the lock is stale.'
    );
  }
  ...
  resolveNexusPath(repoName, relativePath) {
    ...
    if (!repo)
      throw new Error(
        `[ERR-ROCKETMIND-006] Repo ${repoName} not found in Nexus registry. Run: node bin/install.js nexus sync`
      );
  ...
  async executeWave(tasks, waveId = Date.now().toString()) {
    ...
    for (const [index, task] of tasks.entries()) {
      ...
      if (!agent)
        throw new Error(
          `[ERR-ROCKETMIND-004] Agent '${task.agent}' not found in registry at ≥60% match. Run /rocketmind:forge to create a specialist agent.`
        );
      ...
      const taskName = `task_${waveId}_${index}`;
      const branchName = `feat/rocketmind-task-${waveId}-${index}`;
    ...
    try {
      ...
      console.log(
        formatWaveComplete({
          ...
          next: 'Run /rocketmind:verify before shipping',
        })
      );
      console.log(
        formatNextCommand({
          primary: '/rocketmind:verify',
          why: 'Wave output is ready for verification before shipping.',
          alternatives: [
            '/rocketmind:progress — review full milestone status',
            '/rocketmind:resume — reload state if another session updated it',
          ],
        })
      );
      return finalSummary;
...
if (require.main === module) {
  const orchestrator = new RocketMindOrchestrator(process.cwd());
  ...
  if (args[0] === '--wave') {
    ...
    console.log('RocketMind Orchestrator v2.0');
    console.log("Usage: node orchestrator.js --wave=\"[{'agent': 'engineer', 'prompt': '...'}]\"");
  }
}

module.exports = RocketMindOrchestrator;
