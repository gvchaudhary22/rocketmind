...
describe('ensureDir', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-test-'));
  });
...
describe('copyFile', () => {
  ...
  it('copies a file and creates intermediate directories', () => {
    ...
    fs.writeFileSync(src, 'rocketmind-content');
    copyFile(src, dest);
    expect(fs.readFileSync(dest, 'utf8')).toBe('rocketmind-content');
  });
