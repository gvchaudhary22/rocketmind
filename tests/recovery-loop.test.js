...
describe('recovery loop', () => {
  it('records retry decisions before the max-attempt threshold', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-recovery-'));
    const stateDir = path.join(tmpDir, '.rocketmind', 'state');
    const errorDir = path.join(tmpDir, '.rocketmind', 'errors');
  ...
  it('halts after the same error repeats three times', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-recovery-halt-'));
    const stateDir = path.join(tmpDir, '.rocketmind', 'state');
    ...
    expect(payload.next_command).toBe('/rocketmind:review');
  ...
  it('riper runtime surfaces the recovery decision when execute fails', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-riper-runtime-'));
    const output = renderRiper({
      issue: '#147',
      branch: 'feat/147-executable-recovery-loop',
      implementationStatus: 'in_progress',
      task: 'implement recovery loop',
      'state-dir': path.join(tmpDir, '.rocketmind', 'state'),
      'error-dir': path.join(tmpDir, '.rocketmind', 'errors'),
      'error-message': 'TypeError: missing dependency',
    });

    expect(output).toContain('Command:  /rocketmind:riper');
    expect(output).toContain('━━━ Recovery Loop');
    expect(output).toContain('Decision: retry');
    expect(output).toContain('**Primary**: /rocketmind:riper');

    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it('riper runtime automatically invokes recovery when an execute step fails', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-riper-auto-'));
    const stateDir = path.join(tmpDir, '.rocketmind', 'state');
    const output = renderRiper({
      issue: '#147',
      branch: 'feat/147-executable-recovery-loop',
