...
const SAMPLE_STATE_MD = `# RocketMind Project State
...
function makeTmpDb() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-context-test-'));
...
describeIfSqlite('context.js — schema and load levels', () => {
  let db, tmpDir;
  const originalCheckpointPath = process.env.ROCKETMIND_CHECKPOINT_PATH;

  beforeEach(() => {
    ({ db, tmpDir } = makeTmpDb());
    delete process.env.ROCKETMIND_CHECKPOINT_PATH;
  });
  ...
  afterEach(() => {
    ...
    if (originalCheckpointPath === undefined) {
      ...
      process.env.ROCKETMIND_CHECKPOINT_PATH = originalCheckpointPath;
    }
  ...
  it('--load minimal: prioritizes active issue and shows active PR', () => {
    ...
    db.prepare('INSERT INTO tasks (issue_ref, title, status) VALUES (?, ?, ?)').run(
      '#87',
      'name hook stages in rocketmind.config.json',
      'open'
    );
    db.prepare('INSERT INTO tasks (issue_ref, title, status) VALUES (?, ?, ?)').run(
      '#145',
    ...
    try {
      ...
      ).toBeLessThan(output.indexOf('#87 name hook stages in rocketmind.config.json'));
    } finally {
  ...
  it('--load minimal: prefers the matching checkpoint manifest over stale state facts', () => {
    ...
    db.prepare('INSERT INTO tasks (issue_ref, title, status) VALUES (?, ?, ?)').run(
      '#181',
      'auto-chain /rocketmind:quick -> /rocketmind:review -> PR for tracked work',
      'in_progress'
    );
    ...
    process.env.ROCKETMIND_CHECKPOINT_PATH = checkpointPath;
    ...
    try {
      ...
      expect(output).toContain('- Issue: #181 — auto-chain /rocketmind:quick -> /rocketmind:review -> PR for tracked work');
  ...
  it('--load minimal: prefers current-milestone tasks over unrelated legacy backlog noise', () => {
    db.prepare('INSERT INTO state (key, value) VALUES (?, ?)').run(
      'milestone',
      'v2.9.0 — Idea to Market (self-orchestrated by RocketMind)'
    );
    db.prepare('INSERT INTO state (key, value) VALUES (?, ?)').run(
      'phase',
