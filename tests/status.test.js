...
describe('status formatter', () => {
  ...
  it('renders a current execution block', () => {
    ...
    expect(output).toContain('Command:  /rocketmind:quick');
  ...
  it('renders a next command footer', () => {
    const output = status.formatNextCommand({
      ...
      why: 'Wave complete.',
      alternatives: ['/rocketmind:progress'],
    });

    expect(output).toContain('Recommended Next Command');
    expect(output).toContain('**Primary**: /rocketmind:verify');
    expect(output).toContain('- /rocketmind:progress');
  });
