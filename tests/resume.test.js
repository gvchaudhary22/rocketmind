...
describe('resume.sh', () => {
  it('rehydrates from STATE.md when no snapshot exists', () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-resume-state-'));
    const stateDir = path.join(tmpRoot, '.rocketmind', 'state');
    ...
    const output = execFileSync('bash', [RESUME_BIN], {
      ...
      env: {
        ...process.env,
        ROCKETMIND_ROOT_DIR: tmpRoot,
      },
    });

    expect(output).toContain('🔋 Rehydrating RocketMind session...');
    expect(output).toContain('✅ RocketMind rehydrated.');
