...
function buildFakeRuntime({ branch, dirty = false, prData = null }) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-enforcement-e2e-'));
...
function initGitRepo(repoDir) {
  ...
  spawnSync('git', ['config', 'user.name', 'RocketMind Test'], { cwd: repoDir, encoding: 'utf8' });
  spawnSync('git', ['config', 'user.email', 'rocketmind-tests@example.com'], {
    cwd: repoDir,
    encoding: 'utf8',
  });
...
describe('enforcement end-to-end command paths', () => {
  it('progress CLI renders the workflow gate from live git/github evidence', () => {
    ...
    try {
      ...
      expect(output).toContain('Command:  /rocketmind:review');
    } finally {
      runtime.cleanup();
    }
  });
  ...
  it('ship CLI blocks progression when review is still required', () => {
    ...
    try {
      ...
      expect(result.stderr).toContain('/rocketmind:review');
    } finally {
      runtime.cleanup();
    }
  });
  ...
  it('ship CLI allows progression once tests and review are green', () => {
    ...
    try {
      ...
      expect(result.stdout).toContain('/rocketmind:progress');
    } finally {
      runtime.cleanup();
    }
  });
  ...
  it('walks the closed loop from implementation to review gate to PR-open tracking', () => {
    ...
    try {
      ...
      expect(reviewGate.stdout).toContain('Command:  /rocketmind:review');
...
describe('runtime capability generation', () => {
  it('renders different plain-prompt guidance for supported and unsupported runtimes', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-routing-e2e-'));
    ...
    try {
      ...
      expect(codexText).toContain('supports RocketMind workflow inference for plain prompts');
      expect(codexText).toContain('infer the correct RocketMind workflow');
      expect(antigravityText).toContain('does not provide reliable plain-prompt interception');
      expect(antigravityText).toContain("prefer the runtime's documented explicit RocketMind command path");
    } finally {
  ...
  it('writes different adapter contracts for supported and unsupported runtimes', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-contract-e2e-'));
  ...
  it('blocks PR-open progression when the self-review verdict is blocked', () => {
    ...
    try {
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain('RocketMind self-review is blocked');
      expect(result.stderr).toContain('/rocketmind:review');
    } finally {
      runtime.cleanup();
    }
...
describe('install and setup enforcement paths', () => {
  it('installs git hooks in a normal repo through install.sh', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-install-e2e-'));
  ...
  it('installs git hooks in a linked worktree through install.sh', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-worktree-e2e-'));
  ...
  it('setup path still produces an install with active git hooks', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-setup-e2e-'));
    const projectDir = path.join(tmpDir, 'repo');
    initGitRepo(projectDir);
