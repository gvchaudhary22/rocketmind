...
describe('eval-runner issue #85 coverage', () => {
  it('keeps the assertion count above 100', () => {
    const report = runEval();
    expect(report.overall.total).toBeGreaterThanOrEqual(100);
  }, 30000);

  it('covers the v2.9.0 contract surfaces called out in the issue', () => {
    const report = runEval();
    ...
    expect(checks.get('agent contract: business-analyst is registered')).toBe(true);
    expect(checks.get('agent contract: business-analyst includes ## ANTI-PATTERNS')).toBe(true);
    ...
    expect(checks.get('skill contract: skills/instructor.md includes VERIFICATION WORKFLOW')).toBe(
      true
    );
    expect(
    ...
    expect(checks.get('skill contract: skills/user-onboarding.md loaded_by technical-writer')).toBe(
      true
    );
    expect(checks.get('skill contract: skills/user-onboarding.md loaded_by launch-planner')).toBe(
      true
    );
    expect(
      checks.get('skill contract: skills/compliance-checklist.md includes VERIFICATION WORKFLOW')
    ).toBe(true);
    expect(checks.get('skill contract: skills/compliance-checklist.md includes disclaimer')).toBe(
      true
    );
    expect(
      checks.get('skill contract: skills/compliance-checklist.md loaded_by security-engineer')
    ).toBe(true);
    expect(
      checks.get('skill contract: skills/compliance-checklist.md loaded_by launch-planner')
    ).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:ask defines inputs')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:ask references agents')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:ask agent refs exist')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:clarify defines inputs')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:clarify references agents')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:clarify agent refs exist')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:clarify doc exists')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:discover defines inputs')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:discover references agents')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:discover agent refs exist')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:discover doc exists')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:eval defines outputs')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:eval references agents')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:eval agent refs exist')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:riper doc exists')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:riper references agents')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:riper agent refs exist')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:ship references agents')).toBe(true);
    expect(checks.get('workflow contract: /rocketmind:ship includes technical-writer')).toBe(true);
    expect(checks.get('wave 1.5 technical-writer doc stubs exist')).toBe(true);
    expect(checks.get('wave 1.5 technical-writer doc stubs cover wave 1 agents')).toBe(true);
