...
# CI:    docker run --rm -v "$PWD":/rocketmind -w /rocketmind node:22-alpine sh -c "apk add bash && bash tests/install.test.sh"
...
git_init_repo() {
  ...
  (
    ...
    git config user.name "RocketMind Test"
    git config user.email "rocketmind-tests@example.com"
    touch README.md
    git add README.md
...
assert_file "rocketmind.registry.json installed"   "$PROJECT1/.claude/rocketmind.registry.json"
assert_file "adapter.contract.json installed" "$PROJECT1/.claude/adapter.contract.json"
assert_file "rocketmind.config.json installed"     "$PROJECT1/rocketmind.config.json"
...
assert_dir  "hooks/ installed"                "$PROJECT1/.claude/rocketmind/hooks"
assert_file ".rocketmind/state/STATE.md created"   "$PROJECT1/.rocketmind/state/STATE.md"
assert_file ".rocketmind/state/DECISIONS-LOG.md created" "$PROJECT1/.rocketmind/state/DECISIONS-LOG.md"
assert_file ".rocketmind/state/OPERATIONAL-RULES.json created" "$PROJECT1/.rocketmind/state/OPERATIONAL-RULES.json"
...
assert_file "rocketmind.registry.json installed" "$PROJECT2/.codex/rocketmind.registry.json"
assert_file "rocketmind.config.json installed"   "$PROJECT2/.codex/rocketmind.config.json"
...
assert_contains "INSTRUCTIONS.md mentions supported plain-prompt routing" "$PROJECT2/.codex/INSTRUCTIONS.md" "supports RocketMind workflow inference for plain prompts"
...
assert_contains "Claude CLAUDE.md mentions supported plain-prompt routing" "$PROJECT3/.claude/CLAUDE.md" "supports RocketMind workflow inference for plain prompts"
...
assert_file "Node wrapper STATE.md created"       "$PROJECT3B/.rocketmind/state/STATE.md"
assert_file "Node wrapper DECISIONS-LOG.md created" "$PROJECT3B/.rocketmind/state/DECISIONS-LOG.md"
assert_file "Node wrapper OPERATIONAL-RULES.json created" "$PROJECT3B/.rocketmind/state/OPERATIONAL-RULES.json"
...
mkdir -p "$PROJECT5/.rocketmind/state"
echo "# My existing state" > "$PROJECT5/.rocketmind/state/STATE.md"
run_install "$PROJECT5" --tool claude
assert_contains "Existing STATE.md preserved" "$PROJECT5/.rocketmind/state/STATE.md" "My existing state"
...
for hook in pre-tool-use post-tool-use pre-compact stop sync-context post-commit pre-push; do
  assert_exec "hooks/$hook.sh is +x" "$PROJECT7/.claude/rocketmind/hooks/$hook.sh"
done

# ─────────────────────────────────────────────────────────────────────────────
section ".gitignore: RocketMind entries added"
PROJECT8="$TMPDIR_ROOT/proj-gitignore"
mkdir -p "$PROJECT8"
echo "node_modules/" > "$PROJECT8/.gitignore"
run_install "$PROJECT8" --tool claude
assert_contains ".gitignore has .rocketmind/errors/" "$PROJECT8/.gitignore" ".rocketmind/errors/"
