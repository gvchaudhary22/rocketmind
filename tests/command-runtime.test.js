...
function expectParity(output, command) {
  expect(output).toContain('━━━ RocketMind');
...
describe('runtime command status parity', () => {
  ...
  it('quick runtime emits the standard status blocks', () => {
    ...
    expectParity(output, '/rocketmind:quick');
    expect(output).toContain('**Primary**: /rocketmind:ship');
    expect(output).toContain('RocketMind Auto-Chain');
    expect(output).toContain('Final State:  pr_ready');
  });

  it('quick runtime writes a checkpoint manifest and emits the checkpoint block', () => {
    const checkpointDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-runtime-checkpoint-'));
    ...
    expect(output).toContain('RocketMind Checkpoint');
    ...
    expect(latest.orchestration.recommended_next_step).toBe('/rocketmind:ship');
  });
  ...
  it('quick runtime auto-dispatches review after tests go green and updates the chained final state', () => {
    ...
    expect(output).toContain('**Primary**: /rocketmind:review');
  });
  ...
  it('quick runtime routes review findings into a remediation loop', () => {
    ...
    expect(output).toContain('**Primary**: /rocketmind:quick #181');
  });
  ...
  it('quick runtime can execute review and land in remediation_required when findings are returned', () => {
    ...
    expect(result.output).toContain('**Primary**: /rocketmind:quick #181');
  });
    ...
    const result = runQuick(
      ...
      },
      {
        ...
        syncPullRequest: () => ({
          action: 'created',
          number: 187,
          url: 'https://github.com/shiprocket/rocketmind/pull/187',
        }),
      }
    );
  ...
  it('quick runtime creates or updates the PR once the chained state is clean', () => {
    const result = runQuick(
      ...
      },
      {
        syncPullRequest: () => ({
          action: 'created',
          number: 186,
          url: 'https://github.com/shiprocket/rocketmind/pull/186',
        }),
      }
    );
    ...
    expect(result.output).toContain('**Primary**: /rocketmind:progress');
  });
  it('quick runtime blocks when the requested issue does not match the active feature branch', () => {
    ...
    expect(output).toContain('**Primary**: /rocketmind:quick #148');
  ...
  it('quick runtime consults operational rules and blocks the wrong route before execution', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-op-rules-'));
  ...
  it('operational rules match by environment, tool, and operation specificity', () => {
    const rules = loadOperationalRules(
      (() => {
        const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-op-rule-load-'));
    ...
    const match = findOperationalRule(rules, {
      ...
      operation: 'network_mutation',
      route: '',
      runtimeCommand: '/rocketmind:quick',
    });
  ...
  it('plan runtime emits the standard status blocks', () => {
    expectParity(renderPlan(trackedArgs), '/rocketmind:plan');
  });

  it('review runtime emits the standard status blocks', () => {
    expectParity(renderReview(trackedArgs), '/rocketmind:review');
  });

  it('verify runtime emits the standard status blocks', () => {
    expectParity(renderVerify(trackedArgs), '/rocketmind:verify');
  });

  it('next runtime emits the standard status blocks', () => {
    expectParity(renderNext(trackedArgs), '/rocketmind:next');
  });

  it('riper runtime emits the standard status blocks', () => {
    expectParity(renderRiper(trackedArgs), '/rocketmind:riper');
  });

  it('clarify runtime emits the standard status blocks and shows the pending queue', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-clarify-state-'));
    ...
    expectParity(output, '/rocketmind:clarify');
  ...
  it('clarify runtime refuses to resolve a request without an answer', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-clarify-missing-answer-'));
    const stateFile = path.join(tmpDir, 'STATE.md');
    fs.writeFileSync(
      stateFile,
      [
  ...
  it('next runtime resolves the next issue from STATE.md when no tracked branch is active', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-next-state-'));
    const stateFile = path.join(tmpDir, 'STATE.md');
    fs.writeFileSync(
      stateFile,
      [
        '# RocketMind Project State',
        '## Project Context',
        ...
        '- [ ] #150 — feat(workflow): implement `/rocketmind:next` as an executable runtime command',
      ].join('\n')
    );
    ...
    expect(output).toContain(
      '**Primary**: /rocketmind:quick #150 feat(workflow): implement `/rocketmind:next` as an executable runtime command'
    );
  ...
  it('next runtime honors active phase metadata even when the section is not marked CURRENT', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-next-state-phase-'));
    const stateFile = path.join(tmpDir, 'STATE.md');
    fs.writeFileSync(
      stateFile,
      [
    ...
    expect(output).toContain(
      '**Primary**: /rocketmind:quick #151 feat(governance): enforce documentation updates for behavior changes'
    );
  ...
  it('next runtime uses a planning-aligned workflow gate when the active phase backlog is empty', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-next-plan-'));
    const stateFile = path.join(tmpDir, 'STATE.md');
    fs.writeFileSync(
      stateFile,
      [
        ...
        '### v2.9.0 — Idea to Market',
        '#### Enforcement Hardening (CURRENT)',
        '- [x] #150 — feat(workflow): implement `/rocketmind:next` as an executable runtime command',
      ].join('\n')
    );
    ...
    expect(output).toContain('**Primary**: /rocketmind:plan');
  });

  it('next runtime drops an active PR lane once the current branch is already handoff-complete', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-next-handoff-complete-'));
    const stateFile = path.join(tmpDir, 'STATE.md');
    fs.writeFileSync(
      stateFile,
      [
        ...
        '### v2.9.0 — Idea to Market',
        '#### Enforcement Hardening (CURRENT)',
        '- [ ] #62 — feat(state): initialize v2.9.0 release STATE.md — prime RocketMind with full release plan',
      ].join('\n')
    );
    ...
    expect(output).toContain(
      '**Primary**: /rocketmind:quick #62 feat(state): initialize v2.9.0 release STATE.md — prime RocketMind with full release plan'
    );
    expect(output).toContain('State:    issue_ready');
    expect(output).toContain('Working target: Issue #62');
    expect(output).toContain('already in pr_open');
    expect(output).not.toContain('**Primary**: /rocketmind:progress');
  });

  it('prompt dispatch makes explicit rocketmind:quick commands override freeform handling', () => {
    expect(
      classifyPromptDispatch('rocketmind:quick #145', {
        issue: '#181',
        workflowState: 'pr_open',
      })
    ).toEqual({
      type: 'explicit_command',
      command: '/rocketmind:quick',
      raw: 'rocketmind:quick #145',
      remainder: '#145',
      reason:
        'Explicit RocketMind command detected; inferred routing and freeform handling are disabled.',
    });
  });

  it('prompt dispatch makes explicit rocketmind:review commands override active implementation context', () => {
    expect(
      classifyPromptDispatch('rocketmind:review on PR #189', {
        issue: '#145',
        workflowState: 'implementation_done',
      })
    ).toEqual({
      type: 'explicit_command',
      command: '/rocketmind:review',
      raw: 'rocketmind:review on PR #189',
      remainder: 'on PR #189',
      reason:
        'Explicit RocketMind command detected; inferred routing and freeform handling are disabled.',
    });
  ...
  it('prompt dispatch routes pick next task to rocketmind:next instead of staying in the active PR lane', () => {
    ...
    expect(result.command).toBe('/rocketmind:next');
    expect(result.reason).toContain('Next-task prompt detected');
  });

  it('prompt dispatch executes explicit rocketmind:quick through the repo-local runtime path', () => {
    const result = dispatchPrompt(
      'rocketmind:quick #145',
      {
        issue: '#181',
        workflowState: 'pr_open',
      },
    ...
    expect(result.dispatch.command).toBe('/rocketmind:quick');
    expect(result.args.issue).toBe('#145');
    expect(result.output).toBe('executed quick for #145');
  });

  it('prompt dispatch executes explicit rocketmind:review through the repo-local runtime path', () => {
    const result = dispatchPrompt(
    ...
    expect(result.executed).toBe(true);
    expect(result.error).toBe(null);
    expect(result.dispatch.command).toBe('/rocketmind:review');
    expect(result.args.pr).toBe('#189');
    expect(result.output).toBe('executed review for #189');
  });

  it('prompt dispatch executes pick next task through rocketmind:next instead of staying in the current PR lane', () => {
    const result = dispatchPrompt(
      'pick next task',
      {
        issue: '#181',
    ...
    expect(result.executed).toBe(true);
    expect(result.error).toBe(null);
    expect(result.dispatch.command).toBe('/rocketmind:next');
    expect(result.output).toBe('executed next-task routing');
  });

  it('prompt dispatch executes inferred continuation prompts through rocketmind:quick for the active issue', () => {
    const result = dispatchPrompt(
      'go ahead',
      {
        issue: '#181',
    ...
    expect(result.executed).toBe(true);
    expect(result.error).toBe(null);
    expect(result.dispatch.command).toBe('/rocketmind:quick #181');
    expect(result.args.issue).toBe('#181');
    expect(result.output).toBe('continued quick for #181');
  });

  it('prompt dispatch hard-fails unsupported explicit RocketMind commands instead of falling back to manual handling', () => {
    const result = dispatchPrompt('rocketmind:ship', {
      issue: '#181',
      workflowState: 'pr_ready',
    });

    expect(result.executed).toBe(false);
    expect(result.dispatch.command).toBe('/rocketmind:ship');
    expect(result.error).toContain(
      'Explicit RocketMind command /rocketmind:ship is not executable through the repo-local dispatcher yet.'
    );
  });
  ...
  it('early-stage runtime output uses the concrete issue and avoids premature PR blockers', () => {
    ...
    expect(output).toContain('Command:  /rocketmind:quick #146');
    expect(output).not.toContain('/rocketmind:quick #NNN');
