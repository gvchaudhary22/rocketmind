...
describe('workflow state engine', () => {
  ...
  it('treats a tested branch with no review request as tests_green', () => {
    ...
    expect(summary.nextCommand).toBe('/rocketmind:review');
  });
  ...
  it('treats active implementation as implementation_in_progress', () => {
    ...
    expect(summary.nextCommand).toBe('/rocketmind:quick #131');
  ...
  it('keeps a clean tracked branch focused on implementation, not PR blockers', () => {
    ...
    expect(summary.nextCommand).toBe('/rocketmind:quick #146');
    expect(summary.blockers.some((blocker) => blocker.includes('PR state unavailable'))).toBe(false);
  ...
  it('treats merged pull requests as a terminal merged state', () => {
    ...
    expect(summary.nextCommand).toBe('/rocketmind:resume');
  });
  ...
  it('blocks progression when self-review ship decision is blocked', () => {
    ...
    expect(summary.state).toBe('review_required');
    expect(summary.blockers[0]).toContain('RocketMind self-review is blocked');
    expect(summary.prGate).toBe('blocked');
  });
});
