...
describe('checkpoint manifest', () => {
  it('builds the expected manifest shape and writes latest plus history entries', () => {
    const checkpointDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-checkpoint-'));
    const manifest = buildCheckpointManifest({
      ...
      profile: {
        ...
        defaultPrimary: '/rocketmind:ship',
      },
      evidence: {
        issue: '#181',
        branch: 'feat/181-quick-review-pr-autochain',
    ...
    expect(manifest.orchestration.recommended_next_step).toBe('/rocketmind:ship');
