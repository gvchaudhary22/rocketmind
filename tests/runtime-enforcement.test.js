...
describe('runtime enforcement entrypoints', () => {
  ...
  it('ship runtime blocks PR progression when review is incomplete', () => {
    ...
    expect(result.output).toContain('/rocketmind:review');
  });
  ...
  it('ship runtime blocks when GitHub truth is unavailable', () => {
    ...
    expect(result.output).toContain('/rocketmind:quick #131');
  });
  ...
  it('ship runtime allows PR progression when tests and review are green', () => {
    ...
    expect(result.output).toContain('/rocketmind:ship');
  });
  ...
  it('ship runtime blocks when self-review decision is blocked', () => {
    ...
    expect(result.exitCode).toBe(1);
    expect(result.output).toContain('RocketMind self-review is blocked');
    expect(result.output).toContain('/rocketmind:review');
  });
