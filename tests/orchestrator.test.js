...
let RocketMindOrchestrator;
beforeEach(async () => {
  // Dynamically require after vi.resetModules() so mocks are fresh each test
  RocketMindOrchestrator = (await import('../bin/orchestrator.js')).default;
});
...
function buildFixtureRoot(tmpDir) {
  ...
  fs.writeFileSync(path.join(tmpDir, 'rocketmind.registry.json'), JSON.stringify(registry));
  fs.writeFileSync(path.join(tmpDir, 'rocketmind.config.json'), JSON.stringify(config));
  fs.mkdirSync(path.join(tmpDir, '.rocketmind', 'state'), { recursive: true });
  return tmpDir;
}

// ─── Tests ─────────────────────────────────────────────────────────────────

describe('RocketMindOrchestrator — constructor', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-orch-'));
  ...
  it('loads registry and config on construction', () => {
    const orch = new RocketMindOrchestrator(tmpDir);
    expect(orch.registry.agents).toHaveLength(3);
    expect(orch.config.git.worktree_per_task).toBe(false);
  });

  it('isNexus is false when no rocketmind.nexus.json exists', () => {
    const orch = new RocketMindOrchestrator(tmpDir);
    expect(orch.isNexus).toBe(false);
    expect(orch.nexus).toBeNull();
  });

  it('isNexus is true when rocketmind.nexus.json exists', () => {
    const nexus = { workspace_mode: 'nexus', org: 'test-org', repos: [] };
    fs.writeFileSync(path.join(tmpDir, 'rocketmind.nexus.json'), JSON.stringify(nexus));
    const orch = new RocketMindOrchestrator(tmpDir);
    expect(orch.isNexus).toBe(true);
    expect(orch.nexus.org).toBe('test-org');
  });

  it('falls back to default config if rocketmind.config.json missing', () => {
    fs.unlinkSync(path.join(tmpDir, 'rocketmind.config.json'));
    const orch = new RocketMindOrchestrator(tmpDir);
...
describe('RocketMindOrchestrator — state lock', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-lock-'));
    buildFixtureRoot(tmpDir);
  });
  ...
  it('acquireStateLock creates lock directory', () => {
    const orch = new RocketMindOrchestrator(tmpDir);
    const lockPath = path.join(tmpDir, '.rocketmind', 'state', '.rocketmind.lock');
  ...
  it('acquireStateLock throws after 10 failed attempts', () => {
    ...
    const orch2 = new RocketMindOrchestrator(tmpDir);
    expect(() => orch2.acquireStateLock()).toThrow('Could not acquire RocketMind state lock');
    orch.releaseStateLock();
  });
  ...
  it('warns once in CI that the state mutex is local-only', () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining('local .rocketmind.lock directory')
    );
  ...
  it('warns when distributed_mutex_warning is enabled outside CI', () => {
    const configPath = path.join(tmpDir, 'rocketmind.config.json');
...
describe('RocketMindOrchestrator — resolveNexusPath', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-nexus-'));
    buildFixtureRoot(tmpDir);
  });
...
describe('RocketMindOrchestrator — executeWave', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-wave-'));
    buildFixtureRoot(tmpDir);
  });
  ...
  it('emits workflow gate output when tracked work includes an issue', async () => {
    ...
    expect(worktreeSpy).toHaveBeenCalledWith('task_w005_0', 'feat/rocketmind-task-w005-0');
  ...
  it('detects repeated loop signatures and terminates the repeated task', async () => {
    ...
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const stateFile = path.join(tmpDir, '.rocketmind', 'state', 'STATE.md');
    ...
    expect(fs.existsSync(path.join(tmpDir, '.rocketmind', 'state', 'task_loop001_2'))).toBe(false);
...
describe('RocketMindOrchestrator — aggregateResults', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rocketmind-agg-'));
    buildFixtureRoot(tmpDir);
  });
