...
- `/rocketmind:riper` command or explicit mention of RIPER.
...
For `/rocketmind:quick` tasks: RIPER Lite (Research → Plan → Execute → Verify) applies. RALPH fires on any execution error within the Execute step. In the repo-local RIPER runtime, RocketMind activates that recovery loop automatically when the execute step fails.
