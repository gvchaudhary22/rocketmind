...
> Principles for keeping the RocketMind core lean while enabling infinite downstream extension.

## ACTIVATION

Activated when the Architect or Forge detects a need for a new agent/skill, or during the `/rocketmind:promote` workflow.
...
2. **Userland Freedom**: Project-specific specialists live in `.rocketmind/`. They are the "Antibodies" for repo-specific problems.
...
4. **Nexus Awareness**: Before building or promoting, consult the `rocketmind.nexus.json` to see if a similar "Blueprint" (Skill) already exists in the organization.
...
- [ ] If local, is it registered in the project's `.rocketmind/rocketmind.registry.json`?
