...
RocketMind installs shell lifecycle hooks that execute on every AI agent tool call and writes files to user filesystems. We take security reports seriously and aim to respond within 72 hours.

### Preferred Channel

Use [GitHub Private Vulnerability Reporting](https://github.com/shiprocket/rocketmind/security/advisories/new) — this keeps the report confidential until a fix is released.
...
RocketMind's primary attack surface:
...
| `rocketmind.config.json` / `rocketmind.registry.json`        | Config tampering could redirect model routing or disable safety gates       |
| `bin/install.js` — executed as `npx rocketmind init`    | Arbitrary code execution on consumer machines at install time               |

Mitigations in place: `skills/prompt-safety.md`, `agents/safety-evaluator.md`, `hooks/scripts/pre-tool-use.sh` detection layer, and SHASUM manifest (in progress — see [#20](https://github.com/shiprocket/rocketmind/issues/20)).

## Security Best Practices for Consumers

- **Pin your version**: use `@shiprocket/rocketmind@x.y.z` in CI, not `@latest`
...
- Remote code execution via any RocketMind installation or hook vector
