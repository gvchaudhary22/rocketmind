...
RocketMind uses the **GitHub Flow / Release Candidate** branching model. This guide explains the branch structure, protection rules, and release process — for both maintainers and open-source contributors.
...
Run the following command to protect the `develop` branch — all PRs must pass RocketMind Sentinel CI before merging:

```bash
gh api -X PUT /repos/shiprocket/rocketmind/branches/develop/protection --input - <<EOF
{
...
```bash
gh api -X PUT /repos/shiprocket/rocketmind/branches/main/protection --input - <<EOF
{
  "required_status_checks": {
    "strict": true,
    "checks": [
...
4. **Bypass list**: Add **`shiprocket`** (Role: Always).
...
The **RocketMind Release** workflow (`rocketmind-release.yml`) will then:
1. Run `bin/validate.sh` (semantic lint)
2. Extract the latest release notes from `CHANGELOG.md`
3. Publish the package to `@shiprocket/rocketmind` on GitHub Packages
...
- **Pass Sentinel**: All PRs must pass RocketMind Sentinel CI before the "Merge" button activates.
