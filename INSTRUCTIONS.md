...
# RocketMind Instructions

> Canonical operating rules for the RocketMind framework

## Purpose

RocketMind is a repo-native agent orchestration framework. It routes work to specialized agents, applies reusable skills, executes in waves, and persists state across sessions.

## Canonical Sources

- `rocketmind.registry.json` is the machine-readable source of truth for agents, skills, and workflows.
- `rocketmind.config.json` is the runtime configuration source of truth.
...
2. If the runtime supports plain-prompt routing and the request has no explicit `/rocketmind:*` prefix, infer the closest RocketMind workflow before acting.
...
When the active repository is RocketMind itself, use RocketMind workflows to evolve RocketMind. Default to `/rocketmind:quick`, `/rocketmind:plan`, `/rocketmind:build`, `/rocketmind:review`, and `/rocketmind:ship` for framework changes instead of freeform execution.
...
- Use `launch-planner` for: launch plan, go-to-market, gtm, positioning, launch checklist, product launch, release strategy.
- Use `technical-writer` for: api docs, user guide, integration guide, onboarding documentation, changelog entry, readme, release notes, announcement draft, launch messaging, release communication, write documentation, document this.
...
- Read `context.db` first when present, then `.rocketmind/state/STATE.md` as the human-readable fallback.
- Write `.rocketmind/state/STATE.md` after meaningful progress.
- Write `.rocketmind/state/pre-compact-snapshot.md` before compaction or session end.
...
- For RocketMind-on-RocketMind work, start from an issue-backed RocketMind command boundary whenever the task changes framework behavior, docs, hooks, skills, agents, workflows, or runtime adapters.
- When a user gives a plain chat prompt that implies work, planning, review, debugging, or shipping, infer the matching RocketMind workflow only when the active runtime supports plain-prompt routing.
- Treat plain prompts as direct Q&A only when the user is clearly asking for explanation, feedback, or lightweight guidance.
- Run `/rocketmind:eval` or `bash bin/eval.sh` after changes to the control plane, registry, or runtime adapter docs.
...
- PR progression requires evidence in the body: `## Test plan` must list the commands actually run, and `RocketMind Self-Review` must record the review command, dispatched agents, ship decision, and findings handled.
...
- **The Userland**: Forged agents live in project `.rocketmind` directories.
