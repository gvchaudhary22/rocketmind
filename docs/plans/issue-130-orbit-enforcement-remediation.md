---
id: issue-130-rocketmind-enforcement-remediation
doc_type: plan
status: Active
version: v1
...
# RocketMind Enforcement Remediation

> Implementation plan to make RocketMind enforce its workflow contracts at runtime, not only in documentation
...
- #138 tightened plain-prompt routing so RocketMind only promises it on runtimes that actually support it
...
RocketMind should continue from the current branch/PR boundary first, then move through the highest-impact gap-closure stack:
...
7. #150 — executable `/rocketmind:next`
8. #148 — `/rocketmind:eval` contract alignment
9. #147 — recovery-loop enforcement
10. #151 — documentation-update enforcement

This order is intentionally governance-first and state-first so RocketMind becomes harder to misuse before it grows further.

## Why This Exists

Recent review exposed a structural gap in RocketMind:
...
RocketMind is currently stronger as a control-plane specification than as an enforced runtime. This plan exists to close that gap first, before more product-layer features are added on top.

## Core Goal

Move RocketMind from:
...
RocketMind should not merely describe the correct workflow. It should make incorrect progression hard or impossible.
...
This keeps RocketMind from reintroducing the same problem in a new form:
...
- #133 — implement implicit RocketMind workflow routing for plain prompts
...
- [#130](https://github.com/shiprocket/rocketmind/issues/130) — epic(runtime): enforcement-first rocketmind execution remediation
- [#142](https://github.com/shiprocket/rocketmind/issues/142) — epic(enforcement): close documented-vs-enforced workflow gaps
- [#134](https://github.com/shiprocket/rocketmind/issues/134) — fix(runtime): install lifecycle hooks by default and support worktrees
- [#133](https://github.com/shiprocket/rocketmind/issues/133) — feat(runtime): implement implicit RocketMind workflow routing for plain prompts
- [#131](https://github.com/shiprocket/rocketmind/issues/131) — feat(workflow): enforce closed-loop branch-test-review-pr state machine
- [#132](https://github.com/shiprocket/rocketmind/issues/132) — test(enforcement): add end-to-end coverage for setup, routing, and review gates
- [#143](https://github.com/shiprocket/rocketmind/issues/143) — feat(governance): enforce PR freshness and branch governance in CI
- [#144](https://github.com/shiprocket/rocketmind/issues/144) — feat(workflow): require review and ship evidence before progression
- [#145](https://github.com/shiprocket/rocketmind/issues/145) — feat(state): enforce automatic state freshness across command paths
- [#146](https://github.com/shiprocket/rocketmind/issues/146) — feat(runtime): enforce status block parity across documented commands
- [#149](https://github.com/shiprocket/rocketmind/issues/149) — feat(adapters): enforce plain-prompt routing through supported runtimes only
- [#150](https://github.com/shiprocket/rocketmind/issues/150) — feat(workflow): implement `/rocketmind:next` as an executable runtime command
- [#148](https://github.com/shiprocket/rocketmind/issues/148) — feat(eval): align `/rocketmind:eval` with executable artifact contracts
- [#147](https://github.com/shiprocket/rocketmind/issues/147) — feat(recovery): implement executable recovery-loop behavior for documented workflows
- [#151](https://github.com/shiprocket/rocketmind/issues/151) — feat(governance): enforce documentation updates for behavior changes
- [#78](https://github.com/shiprocket/rocketmind/issues/78) — chore(structure): clean repo root + add GENERATED headers + restructure templates

## Root Cause Analysis

### 1. Control Plane Advanced Faster Than The Runtime

RocketMind’s templates, command docs, and generated instruction views evolved faster than the install/setup and adapter layers. That allowed new promises to be merged into the control plane before runtime enforcement existed.

### 2. No Mandatory State Machine

RocketMind has commands and workflow guidance, but not yet a single enforced state model that blocks invalid transitions. This makes review, verification, and PR discipline too easy to bypass.
...
Some features are only real if the adapter can intercept or route user input. Plain-prompt inference is one example. RocketMind needs a clearer distinction between:
...
Goal: make the default setup path activate the lifecycle behaviors RocketMind depends on.
...
- infer the nearest RocketMind workflow for tracked work prompts
- keep explicit `/rocketmind:*` commands higher priority
...
Goal: turn RocketMind’s ideal flow into a progression model with explicit gates.
...
- RocketMind can explain the current state
- RocketMind can explain what is blocking the next state
...
- surface state and blockers in RocketMind status output
...
- RocketMind can report the current state and next allowed transition
...
- the default setup path activates the lifecycle behavior RocketMind documents
- linked worktrees are supported where RocketMind recommends worktrees
...
- RocketMind can enforce the path from issue and branch through test and review to PR
- runtime-eval coverage fails when enforcement behavior drifts from documentation

## Sequencing Policy

This remediation should be completed before major new workflow surface area is added. RocketMind’s foundation must be enforced before the framework grows broader.
