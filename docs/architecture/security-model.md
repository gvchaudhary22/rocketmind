...
# RocketMind Security Model

> How RocketMind defends against supply-chain attacks, prompt injection, and hook abuse

## Threat Model Summary

RocketMind installs lifecycle hooks that execute on every agent tool call. This gives it meaningful system access — and makes it a worthwhile supply-chain target. The following controls are in place.
...
| Hardcoded secrets in commits | `secret_scan_on_commit` guard                  | `rocketmind.config.json`                            |
| Unpatched transitive deps    | npm audit gate + Dependabot weekly updates     | `rocketmind-sentinel.yml`, `.github/dependabot.yml` |
...
rocketmind.registry.json  rocketmind.config.json  rocketmind.config.schema.json
install.sh
agents/*.md
skills/*.md
hooks/scripts/*.sh
...
curl -sL https://github.com/shiprocket/rocketmind/releases/download/vX.Y.Z/SHASUM256.txt -o SHASUM256.txt
...
If you discover a vulnerability in RocketMind, please **do not open a public GitHub issue**. Use GitHub Private Vulnerability Reporting:

[Report a vulnerability →](https://github.com/shiprocket/rocketmind/security/advisories/new)
