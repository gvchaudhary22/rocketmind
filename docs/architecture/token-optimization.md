...
# RocketMind Token Optimization Guide
> How RocketMind minimizes token consumption while maximizing output quality
...
RocketMind solves this with a layered optimization strategy.

---

## Layer 1: Model Routing

Not every task needs the most capable (and expensive) model. RocketMind routes to the minimum-sufficient model:
...
RocketMind's `STATE.md` mutex uses an atomic local directory create (`.rocketmind/state/.rocketmind.lock`). That is safe for one filesystem, but it is not a distributed lock across separate CI runners or remote subagent hosts.
...
- when `CI=true` or `distributed_mutex_warning` is enabled in `rocketmind.config.json`, `bin/orchestrator.js` emits a warning before using the local mutex

This is intentional for now: RocketMind is local-first. The warning exists to prevent a false sense of cross-runner safety, not to replace a real distributed lock service.
...
Instead of feeding full conversation history, RocketMind writes decisions to STATE.md and reads it fresh each session. A new session starts at ~5k tokens instead of 50k+.
