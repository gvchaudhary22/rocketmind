...
> How non-native agent runtimes can use RocketMind without changing the core control plane

## Purpose

RocketMind is designed as a repo-native control plane. The core behavior lives in markdown instructions, reusable skills, workflows, hooks, and a machine-readable registry. A runtime adapter is the thin layer that maps a specific agent engine to that control plane.

## Instruction Generation

RocketMind ships a single source of truth — `templates/rocketmind.base.md` — and generates the runtime-specific instruction file at install time. There are no pre-built per-runtime files in the package.

```
templates/rocketmind.base.md          ← single source of truth (runtime-agnostic)
        ↓ bin/generate-instructions.js --runtime <name>
CLAUDE.md / INSTRUCTIONS.md / …  ← generated at install time for the target runtime
```

Runtimes are configured in `rocketmind.config.json` → `runtimes`. Adding a new runtime requires only a JSON config entry — no new markdown files.
...
- Works with `/rocketmind:` slash commands natively
- Supports plain-prompt RocketMind workflow routing through the native instruction surface
...
Codex does not read `CLAUDE.md`. Instead, RocketMind maps the control plane to Codex's operator surface:

| RocketMind concept        | Claude mapping            | Codex mapping                                           |
| :------------------- | :------------------------ | :------------------------------------------------------ |
| Session orchestrator | `CLAUDE.md`               | `INSTRUCTIONS.md` + `policy.md`                         |
| Agent registry       | `rocketmind.registry.json`     | `rocketmind.registry.json` (same)                            |
| Workflow definitions | `WORKFLOWS.md`            | `WORKFLOWS.md` (same)                                   |
| Slash commands       | `/rocketmind:` via `commands/` | Follow matching `WORKFLOWS.md` section                  |
...
| State persistence    | `.rocketmind/state/STATE.md`   | `.codex/state/STATE.md`                                 |
...
- `policy.md` — injected system context pointing to the RocketMind control plane
- `adapter.contract.json` — executable capability contract for routing and hook support
- `rocketmind.registry.json`, `rocketmind.config.json` — registry and config
...
2. **Agent selection**: Codex reads `rocketmind.registry.json` to select the appropriate agent for the task.
...
4. **Plain prompts**: Codex infers the nearest RocketMind workflow from plain prompts via `INSTRUCTIONS.md` and `policy.md`; explicit `/rocketmind:*` commands still take precedence.
...
Antigravity can follow the RocketMind control plane if it can read markdown instructions and execute repo-local workflows, but no stable hook or lifecycle API has been published. RocketMind now installs a best-effort adapter package, but plain-prompt routing still remains unsupported.

**Current constraints:**

- Best-effort `install_for_antigravity()` exists in `install.sh`, but the runtime contract still prefers explicit RocketMind commands
- Hook injection mechanism is unspecified in Antigravity's public docs
- State persistence pathway is untested
- Plain-prompt RocketMind workflow routing is not currently supported as a reliable runtime capability

**Manual setup (best-effort):**

1. Point Antigravity at `CLAUDE.md` as its operator context.
2. Provide `rocketmind.registry.json` and `WORKFLOWS.md` as reference documents.
3. Write session state manually to `.rocketmind/state/STATE.md`.
...
**Tracking:** Full implementation tracked in [issue #17](https://github.com/shiprocket/rocketmind/issues/17). When Antigravity publishes a stable hook/lifecycle API, `install_for_antigravity()` will be added and the support level will be upgraded to `stable`.
...
Use this checklist when a user asks whether vague/plain prompts should be intercepted by RocketMind first.
...
If the installed contract does not match the runtime expectation, treat that as an RocketMind install/runtime bug.
...
- `native` or `stable` plus `"implicit_prompt_routing": true`:
  - RocketMind should classify the vague prompt first
- `experimental` or `"implicit_prompt_routing": false`:
  - prefer explicit `/rocketmind:*` commands
...
- `pick next task` → `/rocketmind:next` or `/rocketmind:resume`
- `go ahead` after active issue context → `/rocketmind:quick <active issue>`
- `review this` on an active branch → `/rocketmind:review`

Priority rule:

- explicit RocketMind commands such as `rocketmind:quick #145` or `rocketmind:review on PR #189` must be classified as exact-command dispatch before any vague-prompt inference runs
- on supported runtimes, those explicit commands must then execute through the repo-local RocketMind dispatcher path; do not downgrade them into freeform/manual handling for that turn
...
- do not claim RocketMind intercepted the prompt natively
- prefer explicit `/rocketmind:*` commands
...
- inference problem:
  - supported runtime is installed correctly, but RocketMind mapped the prompt to the wrong workflow
