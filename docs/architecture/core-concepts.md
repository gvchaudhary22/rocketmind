...
| `technical-writer`  | PRODUCT, ENGINEERING | API docs, user guides, onboarding docs, changelog and README updates    |
...
API docs / user guide / integration guide / onboarding docs / changelog / README update → technical-writer
...
RocketMind enforces a **Lean Kernel** design. Core agents are the kernel. Project-specific agents created via `/rocketmind:forge` live in `.rocketmind/agents/` and are never committed to the core repo. When a forged agent encodes a genuinely reusable pattern, tag it `promotion_candidate: true` and run `/rocketmind:promote` to upstream it.
...
`STATE.md` is the cross-session memory. Every session reads `.rocketmind/state/STATE.md` first and writes to it on completion. A new session starts with ~5k tokens of full context instead of loading conversation history.
...
When no existing agent covers a task at ≥60% match, `/rocketmind:forge` creates a new specialized agent:
...
5. Register in `CLAUDE.md` and `rocketmind.registry.json`
6. Dispatch task to the new agent
7. Agent persists for all future project tasks

Forged agents are project-local by default (`.rocketmind/agents/`). If the pattern is generalizable, tag with `promotion_candidate: true` and run `/rocketmind:promote`.

---

## Workflows

RocketMind has three complexity tiers, each with a defined workflow:
