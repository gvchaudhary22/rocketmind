...
# RocketMind Architecture

> System design, control plane, wave execution model, and component layout.

### Architecture

RocketMind separates orchestration policy from runtime implementation. The repository defines rules, roles, workflows, and persistence. The active agent runtime interprets those instructions.
...
RocketMind separates orchestration policy from runtime implementation. The repository defines rules, roles, workflows, and persistence. The active agent runtime interprets those instructions. The same repo works with Claude, Codex, or Antigravity — no rewrites required.

```
rocketmind.registry.json     ← machine-readable source of truth (agents, skills, workflows)
rocketmind.config.json       ← runtime config (model routing, hooks, git settings)
...
commands/               ← /rocketmind:* command surface
...
| ----------------- | --------------------------------------------------------- | ---------------------------------- |
| Control Plane     | Routes requests, enforces safety, defines agent roles     | `rocketmind.registry.json`, `CLAUDE.md` |
...
`recommended_parallel_sessions` in `rocketmind.config.json` sets the concurrency target for external orchestration.

## Model Routing

Tasks are routed to the minimum sufficient model. IDs are configured in `rocketmind.config.json → models.routing` — never hardcoded in agent or skill files.
...
Override: edit `rocketmind.config.json → models.routing`. See `examples/model-routing.config.json`.

## Repository Layout

```
CLAUDE.md / INSTRUCTIONS.md   ← session orchestrator (generated from templates/)
rocketmind.registry.json            ← machine-readable agent/skill/workflow registry
rocketmind.config.json              ← runtime config and model routing
rocketmind.config.schema.json       ← JSON schema for config validation
agents/                        ← role-based agent definitions
skills/                        ← reusable process frameworks
hooks/                         ← lifecycle hooks (pre-tool-use, pre-compact, stop)
commands/                      ← /rocketmind:* slash command definitions
docs/plans/                    ← implementation blueprints and wave plans
templates/                     ← rocketmind.base.md source of truth for generated files
templates/                     ← generated instruction + STATE template source files
...
RocketMind enforces a **Lean Kernel** design to prevent bloat.

- **Kernel** — foundational agents (`architect`, `engineer`, `forge`, etc.) and horizontal skills
- **Userland** — project-specific agents created via `/rocketmind:forge` live in `.rocketmind/` and are never committed to core

### Knowledge promotion

When a project-local agent encodes a genuinely reusable pattern, tag it `promotion_candidate: true` and run `/rocketmind:promote` to upstream it to the RocketMind core.

## Nexus Mode (Multi-Repo Orchestration)

Nexus mode lets RocketMind act as a meta-orchestrator across an entire org. It bridges the intelligence gap between independent repos (`api`, `frontend`, `standards`).
...
Every PR is guarded by `.github/workflows/rocketmind-sentinel.yml`:
