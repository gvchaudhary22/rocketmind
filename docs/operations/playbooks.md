...
RocketMind should expose live progress, not just end-state summaries.

- `rocketmind:progress` should show the current command, agent, wave, and status when work is in flight
- tracked work should also surface the workflow gate state, blockers, and next allowed transition
- `rocketmind:resume` should surface a next-command footer after loading state so the user knows what to do next
...
When autonomous execution hits ambiguity that makes continued tool use unsafe, RocketMind should pause instead of guessing.
...
- the active agent emits a `CLARIFICATION_REQUESTED` entry under `## Clarification Requests` in `.rocketmind/state/STATE.md`
- `hooks/scripts/pre-tool-use.sh` blocks further tool execution while open clarification requests exist
- `/rocketmind:clarify` shows the pending queue and can resolve requests so execution can continue
...
When a repeated operational failure pattern is observed, promote it into `.rocketmind/state/OPERATIONAL-RULES.json` instead of relying on conversational memory alone.
...
When autonomous execution repeats the same dispatch pattern without making progress, RocketMind should stop the repeated task instead of spinning forever.

The loop detection contract is:
- `rocketmind.config.json` defines `loop_detection.enabled`, `loop_detection.window_size`, and `loop_detection.threshold`
- `bin/orchestrator.js` tracks repeated dispatch signatures per subagent session within a wave
- once the same session repeats the same signature at or above the threshold, RocketMind emits a `LOOP_DETECTED` runtime event into `.rocketmind/state/STATE.md`
...
```
Step 1: /rocketmind:map-codebase
...
Step 2: /rocketmind:plan — Extraction roadmap
...
```
RocketMind will flag and warn if:
...
Each `rocketmind:quick` or `rocketmind:build` is a discrete task boundary.

- **Follow-up questions / clarifications**: plain prompts are fine.
- **Follow-up work that changes scope**: on runtimes that support plain-prompt routing, RocketMind should infer a new task boundary even if the user does not type `/rocketmind:quick`.
- **Rule of thumb**: if the output should produce a commit, RocketMind should treat the prompt as an RocketMind workflow request when the runtime supports implicit routing; otherwise require the explicit command path.

When in doubt: if you would write a commit message for it, route the prompt through an RocketMind command path, whether explicit or implicit for that runtime.

### Session Switching Protocol

Always run `rocketmind:resume` at the start of each new session. STATE.md is the source of truth — any session that skips resume is working blind.

If no `.rocketmind/state/snapshot.json` exists yet, `rocketmind:resume` should still rehydrate directly from `.rocketmind/state/STATE.md` instead of pretending the project is fresh.

If STATE.md was updated externally while you were in a session (e.g., another session merged a PR and updated STATE.md), run `rocketmind:resume` again mid-session to reload.

```
New session → rocketmind:resume (always)
Switching context mid-session → rocketmind:resume (reload)
Parallel sessions → each runs rocketmind:resume independently
```
...
When you run `/rocketmind:new-project`, RocketMind will:
...
```
/rocketmind:new-project --playbook saas
/rocketmind:new-project --playbook ecommerce
/rocketmind:new-project --playbook ai-agent
/rocketmind:new-project --playbook idp
/rocketmind:new-project --playbook microservices
```
