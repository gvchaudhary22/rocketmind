...
> Naming, placement, metadata, and traceability rules for RocketMind documentation and durable artifacts.

## Goal

RocketMind should make it obvious:
...
Use `.rocketmind/state/` only for runtime state and session artifacts.
...
Use `.rocketmind/reports/` for generated operator-facing runtime artifacts that should not live in the repo root.
...
- control-plane/config artifacts: `rocketmind.registry.json`, `rocketmind.config.json`, `rocketmind.config.schema.json`
...
- `issue-130-rocketmind-enforcement-remediation.md`
...
RocketMind already has historical artifacts such as `docs/plans/v2.9.0-wave-0-release-bootstrap.md`.
