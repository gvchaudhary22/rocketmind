# RocketMind Hooks System
> Automated actions that run before and after key RocketMind events

## Overview
Hooks let you add custom automation around RocketMind operations — linting before commits, Slack notifications after deploys, context validation before execution, and more.

Hooks run as bash scripts in your project root. RocketMind calls them at defined lifecycle points.
...
Add to `.rocketmind/hooks.json` in your project:
...
```json
{
  "hooks": {
    "PreTask": {
      "enabled": true,
      "script": ".rocketmind/hooks/pre-task.sh",
      "timeout": 30,
      "blocking": true
    },
    "PostCommit": {
      "enabled": true,
      "script": ".rocketmind/hooks/post-commit.sh",
      "timeout": 10,
      "blocking": false
    },
    "PreDeploy": {
      "enabled": true,
      "script": ".rocketmind/hooks/pre-deploy.sh",
      "timeout": 120,
      "blocking": true
    },
    "PostDeploy": {
      "enabled": true,
      "script": ".rocketmind/hooks/post-deploy.sh",
      "timeout": 30,
      "blocking": false
    },
    "OnError": {
      "enabled": true,
      "script": ".rocketmind/hooks/on-error.sh",
      "timeout": 15,
      "blocking": false
    }
  }
...
**`blocking: true`** — if the hook exits non-zero, RocketMind stops and reports the failure.
...
if [[ -n "$(git status --porcelain)" ]]; then
  ...
  git stash push -m "rocketmind-pre-task-$(date +%s)"
fi
...
```bash
#!/usr/bin/env bash
# Blocks further tool execution when RocketMind has an unresolved clarification request.

STATE_FILE=".rocketmind/state/STATE.md"

if node bin/clarification-gate.js --state-file "$STATE_FILE" --pending-count | grep -qv '^0$'; then
  echo "[CLARIFICATION] Pending clarification request detected. Run /rocketmind:clarify before continuing."
  exit 2
fi
```

Use this hook when `rocketmind.config.json` sets `clarification_gate: true`.
...
if [[ -n "$SLACK_WEBHOOK_URL" ]]; then
  curl -s -X POST "$SLACK_WEBHOOK_URL" \
    ...
    -d "{
      \"text\": \"🔨 RocketMind committed: \`${COMMIT_HASH}\` — ${COMMIT_MSG} on \`${BRANCH}\`\"
    }" > /dev/null
fi
...
# Signal to RocketMind that rollback is needed
...
```bash
#!/usr/bin/env bash
# Runs when any RocketMind task fails
...
This writes `.rocketmind/state/last_error.json`, appends `.rocketmind/errors/<date>.log`, and returns a deterministic retry-or-halt decision after repeated failures. The repo-local `/rocketmind:riper` runtime uses the same controller automatically when its execute step fails.
...
RocketMind passes these to every hook:
...
```bash
# Add to pre-task.sh
# RocketMind tracks approximate context usage in .rocketmind/state/context.json
if [[ -f ".rocketmind/state/context.json" ]]; then
  USAGE=$(jq '.percentage' .rocketmind/state/context.json 2>/dev/null || echo 0)
  if [[ "$USAGE" -gt 70 ]]; then
    echo "⚠️  Context window at ${USAGE}% — consider /rocketmind:resume in a fresh session after this task"
  fi
  if [[ "$USAGE" -gt 85 ]]; then
    echo "🔴 Context window at ${USAGE}% — RocketMind recommends starting a fresh session now"
    echo "   Run /rocketmind:resume in a new session to continue with full context."
  fi
fi
```
