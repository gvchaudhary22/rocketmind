...
PROJECT_ROOT="${ROCKETMIND_PROJECT_ROOT:-$(cd "$SCRIPT_DIR/../.." && pwd)}"
EVALUATOR="$PROJECT_ROOT/bin/safety-evaluator.js"
STATE_FILE="$PROJECT_ROOT/.rocketmind/state/STATE.md"
CONFIG_FILE="$PROJECT_ROOT/rocketmind.config.json"
...
clarification_bypass_allowed() {
  case "$CHECK_CMD" in
    *"/rocketmind:clarify"*|*"bin/clarify.js"*|*"STATE.md"*)
      return 0
      ;;
    *)
      return 1
...
if clarification_gate_enabled && [ -f "$CLARIFICATION_HELPER" ]; then
  ...
  if [ "${PENDING_COUNT:-0}" -gt 0 ] && ! clarification_bypass_allowed; then
    echo "[CLARIFICATION] Pending clarification request detected. Run /rocketmind:clarify before continuing tool execution." >&2
    exit 2
  fi
fi
