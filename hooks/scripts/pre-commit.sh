#!/usr/bin/env bash
# RocketMind Pre-Commit Hook — Enforces governance before any local commit
...
echo "🛡️  RocketMind Governance: Running bin/validate.sh before commit..."

# 1. Run the validator. Must pass 100%.
if ! bash "$ROOT_DIR/bin/validate.sh" > /dev/null; then
  echo "❌ COMMIT BLOCKED: RocketMind validator failed. Fix the semantic errors or missing files first." >&2
...
echo "🛡️  RocketMind Governance: Scanning for secrets..."
...
echo "✅ RocketMind Governance: Validation passed. Proceeding with commit."
exit 0
