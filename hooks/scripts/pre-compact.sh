#!/usr/bin/env bash
# RocketMind PreCompact Hook — Saves session state before Claude Code compacts the context window
...
STATE_DIR="${AGENTIC_FRAMEWORK_STATE_DIR:-.rocketmind/state}"
...
# Write compact snapshot — this is loaded by /rocketmind:resume in the next session
cat > "$SNAPSHOT_FILE" << SNAPSHOT
# RocketMind Pre-Compact Snapshot
...
## Active Work
<!-- RocketMind: Append your current in-progress task summary here if you're mid-task -->
...
## Resume Instructions
1. Run \`/rocketmind:resume\` in the new session
...
echo "RocketMind: Pre-compact snapshot saved to $SNAPSHOT_FILE" >&2
exit 0
